...
function set_content($text,$spider='0'){
    ...
    if(preg_match($img_re, $text)){
       if(!$spider){
            $text = preg_replace($img_re, '<a href="\1" target="_blank"><img src="\1" class="loading" /></a>', $text);
        }else{
            // 搜索引擎来这样显示 更利于SEO 参见 http://saepy.sinaapp.com/t/81
            $text = preg_replace($img_re, '<a href="\1" target="_blank"><img src="\1" /></a>', $text);
        }
    }
    // 腾讯微博图片
	...
	//下载图标格式化
	if(strpos($text, 'www.eoen.org')){
        $text = preg_replace('/http:\/\/www\.eoen\.org\/upload\/files\/([a-zA-Z0-9\=]+)\.zip/', '<a href="https://www.eoen.org/upload/files/\1.zip" target="_blank" class="btndownlosd"><i class="fa fa-download"></i> &nbsp;下载附件 (ZIP)</a>', $text);
    }
	
	if(strpos($text, 'www.eoen.org')){
        $text = preg_replace('/http:\/\/www\.eoen\.org\/upload\/files\/([a-zA-Z0-9\=]+)\.rar/', '<a href="https://www.eoen.org/upload/files/\1.rar" target="_blank" class="btndownlosd"><i class="fa fa-download"></i> &nbsp;下载附件 (RAR)</a>', $text);
    }
	
	if(strpos($text, 'www.eoen.org')){
        $text = preg_replace('/http:\/\/www\.eoen\.org\/upload\/files\/([a-zA-Z0-9\=]+)\.apk/', '<a href="https://www.eoen.org/upload/files/\1.apk" target="_blank" class="btndownlosd"><i class="fa fa-download"></i> &nbsp;下载附件 (APK)</a>', $text);
    }
