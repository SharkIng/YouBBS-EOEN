...
<p>',$t_obj['content'],'</p>
',$options['ad_post_bot'],'
<div class="topic-title-date"></div><div class="tagge">';
if($t_obj['favorites']){
    echo '<i class="fa fa-star"></i> ',$t_obj['favorites'],' 收藏';
}
if($in_favorites){
        echo '&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-star-o"></i> <a href="/favorites?act=del&id=',$t_obj['id'],'" title="点击取消收藏">取消收藏</a>';
    }else{
        echo '&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-star"></i> <a href="/favorites?act=add&id=',$t_obj['id'],'" title="点击收藏">收藏</a>';
    }
    if($cur_user['flag']>=99){
        echo '&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-pencil-square"></i> <a href="/admin-edit-post-',$t_obj['id'],'">编辑</a>';
    }
echo'<span class="right"><i class="fa fa-eye"></i> ',$t_obj['views'],' 阅读</span></div></div>
</div>
...
if($t_obj['closecomment']){
	if($t_obj['cid'] == 3){
    echo '<div class="no-comment">社区沙盒内的主题禁止评论</div>';
	}else{echo '<div class="no-comment">该主题评论已关闭</div>';
	}
}else{
...
<div class="main-box">';
if($tip){
    echo '<p class="red"><i class="fa fa-info-circle"></i> ',$tip,'</p>';
}
